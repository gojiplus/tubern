...
# get_report(ids = "channel==MINE", metrics = "vews", start_date = "last_7_days") # nolint
...
df_orig <- yt_as_data_frame(report, clean_names = FALSE)
```
...
demographics <- get_audience_demographics("last_90_days")
demo_df <- yt_as_data_frame(demographics)
...
device_df <- yt_as_data_frame(devices)

# Geographic analysis
geography <- get_geographic_performance("last_90_days", max_results = 50)
geo_df <- yt_as_data_frame(geography)
...
videos_df <- yt_as_data_frame(videos_monthly)

# Compare with previous month
videos_prev <- get_top_videos("last_month", max_results = 50)
videos_prev_df <- yt_as_data_frame(videos_prev)
...
trends_df <- yt_as_data_frame(daily_trends)
...
- Use `yt_diagnose()` to troubleshoot authentication issues
...
### 3. Error Handling
- Always check `yt_diagnose()` if you encounter issues
- Use `check_api_quota()` to monitor your API usage
- Enable `setup_guide = TRUE` for first-time setup help

### 4. Data Analysis
- Convert API responses to data.frames with `yt_as_data_frame()`
...
- [tubern GitHub Repository](https://github.com/gojiplus/tubern) for issues and updates

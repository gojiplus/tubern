...
yt_oauth <- function (app_id = NULL, app_secret = NULL,
  ...
  if (file.exists(token)) {
    ...
    if (inherits(google_token, "try-error")) {
      stop(tubern_error(
        message = sprintf("Unable to read token from: %s\nThe token file may be corrupted. Delete it and run yt_oauth() again.", token),
        type = "auth"
      ))
    }
    ...
    tryCatch({
      if (!is.null(google_token) && !is.null(google_token$credentials)) {
        message("Using existing authentication token")
        options(google_token = google_token)
        return(invisible())
      }
    }, error = function(e) {
      warning("Existing token appears invalid, creating new one...")
    })
  ...
  if (!grepl("\\.apps\\.googleusercontent\\.com$", app_id)) {
    warning("app_id should end with '.apps.googleusercontent.com'")
  }
  ...
  message("Setting up OAuth with scope: ", scope)
  ...
  google_token <- tryCatch({
    ...
    stop(tubern_error(
      message = paste("OAuth setup failed:", e$message, 
  ...
  tryCatch({
    ...
    warning("Token created but validation failed. You may need to enable YouTube Analytics API in Google Cloud Console.")
  })
}
...
.get_oauth_scopes <- function(scope) {
  switch(scope,
    ...
    stop("Invalid scope. Use 'analytics', 'monetary', or 'auto'")
  )
}
